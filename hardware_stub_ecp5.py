# hardware_stub_ecp5.py — Hardware protocol stub for Lattice ECP5 Evaluation Board
#
# Reads binary CNF packets, ignores the formula, returns hardcoded UNSAT (0x00).
# This is the hardware equivalent of stub_core.py: it exercises the full wire
# protocol (host → FPGA → host) without implementing a real SAT solver.
#
# Protocol (host → FPGA):
#   0xAA               — start marker
#   num_vars  (1 byte) — number of variables   (ignored by stub)
#   num_clauses(1byte) — number of clauses      (ignored by stub)
#   <literals, 0x00 clause terminators, ...>    (ignored by stub)
#   0xFF               — end marker  (triggers response)
#
# Protocol (FPGA → host):
#   0x00               — RESULT_UNSAT (hardcoded)
#
# Expected output from sat_experiment.py:
#   Result : {'result': 'UNSAT'}
#
# LED indicators:
#   LED0 (A13) — RX active (receiving a byte)
#   LED1 (A12) — TX active (sending response)
#   LED2 (B19) — packet in progress (between 0xAA and 0xFF)
#
# Wiring (J39 header):
...
#   CP2102 GND -> J39 GND
#
# Build:   python hardware_stub_ecp5.py
...
class Top(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        # pending: set when a complete packet arrives; cleared when tx picks it up
        pending = Signal()

        m.d.comb += [
            rx.rx.eq(uart_pins.rx.i),
            uart_pins.tx.o.eq(tx.tx),
            tx.data.eq(RESULT_UNSAT),
        ...
        with m.FSM():
            with m.State("WAIT_START"):
                with m.If(rx.valid & (rx.data == START_MARKER)):
                    m.next = "DRAIN"

            with m.State("DRAIN"):
                m.d.comb += led_busy.o.eq(1)
                with m.If(rx.valid & (rx.data == END_MARKER)):
